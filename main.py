...
def introImg(x, y):
    intro = pygame.image.load("im_gs\intro_img.png")

    screen.blit(intro, (x, y))


def instructionIMG(x, y):
    instruct = pygame.image.load("im_gs\instruction_s.png")
    run = True
    while run:
...
def aboutIMG(x, y):
    aboutimg = pygame.image.load("im_gs/ab_out.png")
    run = True
    while run:
...
def ABOUT(x, y):
    aboutText = IntroFont.render("ABOUT", True, (255, 255, 255))
    screen.blit(aboutText, (x, y))


def Instruction(x, y):
    instructionText = IntroFont.render("INSTRUCTION", True, (255, 255, 255))
    screen.blit(instructionText, (x, y))


def introscreen():
    run = True
    pygame.mixer.music.load('images/startingMusic.mp3')
    ...
    while run:
        ...
        pygame.draw.rect(screen, (0, 0, 0), button2, 6)
        pygame.draw.rect(screen, (0, 0, 0), button3, 6)
        ...
        if button2.collidepoint(x, y):
            ### changing from inactive to active by changing the color from white to red
            pygame.draw.rect(screen, (255, 255, 0), button2, 6)
        ...
        if button3.collidepoint(x, y):
            ### changing from inactive to active by changing the color from white to red
            pygame.draw.rect(screen, (255, 255, 0), button3, 6)
